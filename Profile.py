...
# THIS CODE RIGHT NOW, though can you certainly take a look at it if you are
# curious since we
...
code. It is raised when attempting to load or save Profile objects
to file the system.
...
own code. It is raised when attempting to deserialize
...
class Profile:
    ...
    properties exposed by this class. A Profile class should ensure that
    a username and password are set, but contains no conventions to do so.
    ...
    multiple Posts objects are created, but added to the Profile in a
    different order, it is possible for the list to not be sorted by the Post.
    ...
    operation on the posts returned from the
    ...
    def save_profile(self, path: str) -> None:
        ...
        if p.exists() and p.suffix == '.dsu':
            try:
                ...
                raise DsuFileError("Error trying to process the DSU file.", ex)
        else:
